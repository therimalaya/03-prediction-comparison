...
The datasets discussed in previous section a) _error dataset_ and b) _component dataset_ are used on a MANOVA model. Let us call them _error model_ and _component model_ respectively. The models are fitted with the third order interaction of simulation parameters (`p`, `gamma`, `eta` and `relpos`) and `Methods` as in \@ref(eq:expanded-model).
