...
(\#eq:pred-error)
\end{equation}
where, $\boldsymbol{\Sigma}_{xx}$ is the true covariance matrix of predictor and $\sigma_{y_j\mid x}$ is the true model error obtained from simulation of response $j = 1, \ldots m$. \highlight{Here prediction error is scaled by the true model error to remove its effect on prediction error.} (\alert{Need better reason.}) The prediction error in \@ref(eq:pred-error) is computed for all replications of `r nrow(design)` designs.
