...
as a prediction error for a given design, method, replication and number
of components, where, \(\boldsymbol{\Sigma}_{xx}\) is the true
...
Here prediction error is scaled by the true model error to remove the
effects of influencing residual variances from the prediction error so
that a proper comparison is possible
\alert{|still need some suggestion|}.
