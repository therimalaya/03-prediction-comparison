## Prepare the data ---------
prepare_data <- function(x, y, prop = 0.975, ncomp = 10) {
  ret <- new.env()
...
fit_model <- function(data, method = c("PCR", "PLS1", "PLS2", "Xenv", "Senv"), ncomp, scale = FALSE, ...) {
...
## Compute Intercept ----
compute_intercept <- function(beta_coef, x_mean, y_mean) {
...
get_beta_fn <- function(method = c('pcr', 'pls', 'mvr', 'xenv', "senv", 'try-error', "pls1", 'pls2')){
...
rotate_coef <- function(coef_mat, rotation_mat) {
...
get_prediction <- function(x, coef) {
...
plot_relcomp <- function(relcomp_df, ncomp = 20, title_extra = NULL) {
  relcomp_df %>% filter(Components <= ncomp) %>% 
    ggplot(aes(Components, covariance)) +
    geom_bar(aes(y = Eigenvalues), stat = "identity",
             fill = "lightgrey") +
    geom_line(group = 1) + 
