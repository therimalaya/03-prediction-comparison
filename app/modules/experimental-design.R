...
simrel_plotsUI <- function(id, which, height = '300px') {
  ns <- NS(id)
  plotOutput(ns(paste0('simrel_plot_', which)), height = height)
}
...
simrel_plots <- function(input, output, session, selected_design) {
  sobj <- selected_design %>% simulate()
